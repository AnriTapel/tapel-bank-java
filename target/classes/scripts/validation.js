...
function validateClientPassport(evt) {
    var code = $("#client-passport").val();
    if (evt.keyCode === 8) {
        code = code[code.length - 1] === " " ? code.substr(0, code.length - 1) : code;
        $("#client-passport").val(code);
        return;
    }// Check that current key is numeric
    else if (/^[a-zA-Z]*$/.test(parseInt(evt.key)) || /^[а-яА-Я]*$/.test(parseInt(evt.key)) || code.length > 11)
        code = code.substr(0, code.length - 1);
